...
def weightPlus(condition):
    ...
    examplesThatSatisfyCondition = getExamplesThatSatisfyCondition(
        condition, trainingDataSet)
    ...
    # FIVE
    # print " ***** Condition: %s weightPlusSum: %s (from %s examples)" %
    # (condition, weightPlusSum, len(positivelyLabeledExamples))
...
def weightMinus(condition):
    ...
    # FIVE
    # print " ***** weightMinusSum: %s (from %s examples)" %
    # (str(weightMinusSum), len(negativelyLabeledExamples))
...
def calculateZ(d1, d2):
    ...
    firstPart = math.sqrt(
        weightPlus(andConditions(d1, d2)) * weightMinus(andConditions(d1, d2)))
    secondPart = math.sqrt(weightPlus(andConditions(d1, notCondition(d2)))
                           * weightMinus(andConditions(d1, notCondition(d2))))
...
def outputCalculateZ(d1, d2):
    ...
    # print "Second part b: " , weightMinus( andConditions( d1,
    # notCondition(d2) ) )
    secondPart = math.sqrt(weightPlus(andConditions(d1, notCondition(d2)))
                           * weightMinus(andConditions(d1, notCondition(d2))))
    print "secondPart: ", secondPart

    print "Third part: ", weight(notCondition(d1), False)
    print "d1              : ", d1
...
def updateWeights(rule, costPlus, costMinus):
    for exampleIdentifier in trainingDataSet.keys():
        ...
        if classification == SAME:
            ...
            yi = -1
        cost = costPlus * \
            calculateI(yi == 1) + costMinus * calculateI(yi == -1)
        weights[exampleIdentifier] = cost * weights[
            exampleIdentifier] * math.pow(math.e, -yi * score)
    return weights
...
def adt(costPlus, costMinus, numTreeNodes):
    ...
    initialAlpha1 = 0.5 * \
        math.log((costPlus * weightPlus('True') + smoothFactor)
                 / (costMinus * weightMinus('True') + smoothFactor))
    prediction_node_title = "node0 = " + str(initialAlpha1)

    initialAlpha2 = 0
    initialSplitterAndAssociatedPNodes = sNodeAndItsAssociatedPNodes(
        'True', 'True', initialAlpha1, initialAlpha2)
    ...
    for i in range(numTreeNodes):
        ...
        # Set alpha1 and alpha2 which are the scores of positive and negative
        # classification on all training examples that meet the d1, d2
        # conditions
        print "smoothFactor: ", smoothFactor
        alpha1 = 0.5 * math.log((costPlus * weightPlus(andConditions(d1, d2)) + smoothFactor) / (
            costMinus * weightMinus(andConditions(d1, d2)) + smoothFactor))
        ...
        alpha2 = 0.5 * math.log((costPlus * weightPlus(andConditions(d1, notCondition(d2))) + smoothFactor) / (
            costMinus * weightMinus(andConditions(d1, notCondition(d2))) + smoothFactor))
        ...
        # Set rules = rules + new sNodeAndItsAssociatedPNodes with d1 is
        # precondition, d2 is condition, scores alpha1 for yes and alpha2 for
        # no
        rules.append(sNodeAndItsAssociatedPNodes(d1, d2, alpha1, alpha2))
        nodeNum, yesOrNo = getNodeNumAndYesOrNoCondition(
            dictOfNodeAndItsConditions, d1)
        ...
        node_info = "node " + \
            str(i + 1) + " (cond:" + str(d2) + '(y: ' + str(alpha1)[:4] + ' n: ' + str(
                alpha2)[:4] + ")) is attached to the " + yesOrNo + " of node " + str(nodeNum)
        print "node_info:", node_info

        # print "condition selected:", str(d2)
        # remove the most recently selected condition ... which is str(d2) ...
        # from allConditions
        ...
        '''
        if str(nodeNum) == "0":
            tree_nodes_string = "node" + str(i + 1) + "#(" + str(d2) + ")#" + str(
                alpha1)[:4] + "#" + str(alpha2)[:4] + "#" + prediction_node_title
        else:
            tree_nodes_string = "node" + str(i + 1) + "#(" + str(d2) + ")#" + str(
                alpha1)[:4] + "#" + str(alpha2)[:4] + "#" + "node" + str(nodeNum) + yesOrNo
        ...
        dictOfNodeAndItsConditions[i + 1] = [
            andConditions(d1, d2), andConditions(d1, notCondition(d2))]
        # update all weights
...
def evaluate(parmTestDataSet, rule):
    ...
    # Run through rule and see if the conditions are met. Along the way add up
    # the scores.
    outputDatabase = {}
        # ex. outputDatabase['1-2']['classification'] == SAME and
        # outputDatabase['1-2']['score'] == -0.37
    for exampleIdentifier in testDataSet.keys():
        score = getScoreOfExample(rule, testDataSet[exampleIdentifier])
        # If total Score is negative, return negative classification or visa
        # versa
        if score >= 0:
            outputDatabase[exampleIdentifier] = {
                'classification': SAME, 'score': score}
        else:
            outputDatabase[exampleIdentifier] = {
                'classification': DIFFERENT, 'score': score}
...
def classifier(parmTrainingDataSet, numTreeNodes, parmAllConditions):
    ...
    costPlus = 2.0
    costMinus = 1.0
    # Run adt algorithm and return the tree (in the form of the last rule
    # generated)
